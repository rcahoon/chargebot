...
*/
	...
	Function HMAC($function, $data, $key, &$encoded)
	{
		switch($function)
		{
			case 'sha1':
			...
			default:
				...
				return $this->SetError($function.' is not a supported HMAC hash type');
		}
		if(strlen($key) > 64)
			$key = pack($pack, $function($key));
		if(strlen($key) < 64)
			$key = str_pad($key, 64, "\0");
		$encoded = pack($pack, $function((str_repeat("\x5c", 64) ^ $key).pack($pack, $function((str_repeat("\x36", 64) ^ $key).$data))));
		return(true);
	}
	...
	Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		if(IsSet($oauth))
		{
			...
			switch($this->signature_method)
			{
				...
				case 'HMAC-SHA1':
					...
					if (!$this->HMAC('sha1', $sign, $key, $sigen))
						return(false);
					$values['oauth_signature'] = base64_encode($sigen);
					break;
				default:
		...
		else
		{
			...
			{
				return $this->SetError($this->access_token_error);
			}
		}
		return(true);
	}
	...
	Function ProcessToken($code, $refresh)
	{
		...
		if(strlen($this->access_token_error))
		{
			...
			return $this->SetError($this->authorization_error); //true
		}
...
*/
	Function Initialize()
	{
		if(strlen($this->server) === 0)
			return $this->SetError('Authentication server has not been set');
						...
						else
						{
							...
							switch($method)
							{
								...
								case 'POST':
									...
									break;
								default:
									return $this->SetError($method.' is not a supported method to request tokens');
							}
